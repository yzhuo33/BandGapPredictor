...
classification = RandomForestClassifier(n_jobs=-1, n_estimators=50, max_features=12,
                                        random_state=100).fit(X_train_c, Y_train_c)

# Estimation of classification model performance
y_pred_c = classification.predict(X_test_c)
...
# Random forest model construction
regression = RandomForestRegressor(n_jobs=-1, n_estimators=100, max_features=25,
                                   random_state=100, ).fit(X_train_r, Y_train_r)
# predicted_Y1_r = regression.predict(X_train_r)

# Estimation of regression model performance
y_pred_r = regression.predict(X_test_r)
